...
public class BeanUtils extends org.apache.commons.beanutils.BeanUtils {
	...
	public static Class getBeanGernaricParmeterType(Class beanClass, int index) {
		...
		if (beanClass.getGenericSuperclass() instanceof ParameterizedType) {
			...
			logger.debug("==================="
					+ ptype.getRawType().getClass().getName());
...
//		}
		...
		//System.out.println(numStr+"=="+value.toString());
//		Double value2 = new Double(numStr);
//		if(value2.doubleValue()==value.doubleValue()){
//			System.out.println(numStr+"===================="+value.doubleValue());
//		}else{
//			System.out.println("----------------------");
//		}
		//if(1==1)return;
		...
		if(null!=numStr&&pos>0){
			...
			if(value.doubleValue()!=new Double(value.toString())){
				System.out.println("超出精度!!");
			}
			if(fSize>0){
			   System.out.print(fSize+"位小数");
			}else{
				 System.out.print("整数");
			}
			if(1==1)return;
		...
		nf.setMaximumFractionDigits(8);
		nf.setGroupingUsed(false);
		System.out.println(nf.format(new Double(123)));
		...
		if(ob instanceof java.lang.Number) {
			...
			nf.setMaximumIntegerDigits(30);
			nf.setMaximumFractionDigits(8);
			System.out.println(nf.format(ob));
		}
		class VOTest {
		...
		try {
			System.out.println(DateUtils.getDefautFormat(dt.toLocaleString()));
